...
final class AuthViewController: UIViewController, WebViewViewControllerDelegate
{
    ...
    private func addConstraint() {
        ...
        NSLayoutConstraint.activate([
            ...
            button.bottomAnchor.constraint(
                equalTo: view.bottomAnchor, constant: -124),
            ...
            DispatchQueue.main.async {
                ...
                case .success(let token):
                    ...
                    self.delegate?.authViewController(
                        self, didAuthenticateWithCode: token)
                case .failure(let error):
                    print("[fetchToken] error from receiving token is \(error)")
                    self.showAlert(
                        title: "ОШИБКА",
                        message: "Не удалось получить код доступа")
                }
            }
        }
    }
    ...
    @objc private func onButtonTapped() {
        ...
        guard
            let webViewViewController = storyboard.instantiateViewController(
                withIdentifier: "WebViewViewController")
                as? WebViewViewController
        else {
            fatalError(
                "Failed to instantiate WebViewViewController from Storyboard")
        }
